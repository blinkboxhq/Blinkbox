...
export default function OneDriveTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="OneDrive" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">OneDrive</span>
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                Microsoft Graph subscriptions push OneDrive changes to your BlinkBox URL.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Drive ID</label>
              <input value={config.driveId || ''} onChange={e => updateConfig?.('driveId', e.target.value)}
                placeholder="leave blank for personal drive" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
        ...
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.changeType','Type of change'],['$trigger.resourceId','Resource ID'],['$trigger.fileName','File name'],['$trigger.webUrl','Browser link to file'],['$trigger.lastModifiedTime','Last modified timestamp'],['$trigger.createdTime','Creation timestamp']].map(([k,d]) => (
