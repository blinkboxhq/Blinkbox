...
async function opListFiles(config, { headers: h, siteId }) {
  ...
  const { data } = await axios.get(`${base}/items/${encodeURIComponent(folderId)}/children`, { headers: h, timeout: 15000 });
...
async function opUploadFile(config, { headers: h, siteId, input }) {
  ...
  const { data } = await axios.put(`${base}/root:/${encodeURIComponent(fileName)}:/content`, content, { headers: { ...h, "Content-Type": "text/plain" }, timeout: 30000 });
...
async function opDownloadFile(config, { headers: h, siteId, input }) {
  ...
  const { data: meta } = await axios.get(`${GRAPH}/sites/${encodeURIComponent(siteId)}/drive/items/${encodeURIComponent(itemId)}`, { headers: h, timeout: 10000 });
...
async function opSearchFiles(config, { headers: h, siteId, input }) {
  ...
  const { data } = await axios.get(`${GRAPH}/sites/${encodeURIComponent(siteId)}/drive/root/search(q='${encodeURIComponent(q)}')`, { headers: h, timeout: 15000 });
...
async function opDeleteFile(config, { headers: h, siteId, input }) {
  ...
  await axios.delete(`${GRAPH}/sites/${encodeURIComponent(siteId)}/drive/items/${encodeURIComponent(itemId)}`, { headers: h, timeout: 15000 });
...
async function opGetFileMetadata(config, { headers: h, siteId, input }) {
  ...
  const { data } = await axios.get(`${GRAPH}/sites/${encodeURIComponent(siteId)}/drive/items/${encodeURIComponent(itemId)}`, { headers: h, timeout: 10000 });
...
async function opUpdateFileContent(config, { headers: h, siteId, input }) {
  ...
  const content = config.content ?? input.content ?? "";
  const { data } = await axios.put(`${GRAPH}/sites/${encodeURIComponent(siteId)}/drive/items/${encodeURIComponent(itemId)}/content`, content, { headers: { ...h, "Content-Type": "text/plain" }, timeout: 30000 });
  return { id: data.id, name: data.name, webUrl: data.webUrl, size: data.size };
}
...
async function opCopyFile(config, { headers: h, siteId, input }) {
  ...
  const res = await axios.post(`${GRAPH}/sites/${encodeURIComponent(siteId)}/drive/items/${encodeURIComponent(itemId)}/copy`, body, { headers: h, timeout: 15000 });
...
async function opMoveFile(config, { headers: h, siteId, input }) {
  ...
  if (config.newName) body.name = config.newName;
  const { data } = await axios.patch(`${GRAPH}/sites/${encodeURIComponent(siteId)}/drive/items/${encodeURIComponent(itemId)}`, body, { headers: h, timeout: 15000 });
