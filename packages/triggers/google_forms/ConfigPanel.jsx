...
export default function GoogleFormsTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Google Forms" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Google Forms</span>
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                Google Forms response webhooks require Apps Script. Create a trigger in Tools → Script editor → onFormSubmit.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Form ID</label>
              <input value={config.formId || ''} onChange={e => updateConfig?.('formId', e.target.value)}
                placeholder="Google Form ID from URL" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
          <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
            <p className="text-[9px] text-zinc-500 leading-relaxed">No additional filter options. All form submissions will trigger this node.</p>
          </div>
        )}
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.responseId','Unique response ID'],['$trigger.answers','Array of answers'],['$trigger.submittedAt','Submission timestamp'],['$trigger.formId','Form ID'],['$trigger.formTitle','Form title']].map(([k,d]) => (
