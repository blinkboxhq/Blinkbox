...
export default function ProductHuntTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Product Hunt" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Product Hunt</span>
      ...
      <div className="p-3 flex flex-col gap-3">
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                Product Hunt uses a polling approach. Configure the API key to fetch latest launches.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <input value={config.apiKey || ''} onChange={e => updateConfig?.('apiKey', e.target.value)}
                placeholder="Product Hunt developer API key" type="password" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
          <div className="flex flex-col gap-1.5">
            <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Category Filter</label>
            <select value={config.category || 'all'} onChange={e => updateConfig?.('category', e.target.value)}
        ...
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.id','Product ID'],['$trigger.name','Product name'],['$trigger.tagline','Product tagline'],['$trigger.votesCount','Upvote count'],['$trigger.url','Product Hunt URL'],['$trigger.thumbnail','Thumbnail image URL'],['$trigger.publishedAt','Launch date']].map(([k,d]) => (
