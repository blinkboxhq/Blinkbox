...
async function opAddContact(config, token) {
  ...
  const response = await axios.put(`${BASE}/marketing/contacts`, body, { headers: auth(token), timeout: 120000 });
...
async function opGetContact(config, token) {
  ...
  if (config.contactId) {
    const response = await axios.get(`${BASE}/marketing/contacts/${encodeURIComponent(config.contactId)}`, { headers: auth(token), timeout: 120000 });
    return { contact: response.data };
  }
  const response = await axios.post(`${BASE}/marketing/contacts/search/emails`, { emails: [config.email] }, { headers: auth(token), timeout: 120000 });
...
async function opSearchContacts(config, token) {
  ...
  const response = await axios.post(`${BASE}/marketing/contacts/search`, { query: config.query }, { headers: auth(token), timeout: 120000 });
...
async function opDeleteContact(config, token) {
  ...
  const response = await axios.delete(`${BASE}/marketing/contacts`, { headers: auth(token), params: { ids: config.contactId }, timeout: 120000 });
