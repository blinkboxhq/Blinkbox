...
async function opUploadText(config, token) {
  ...
  const res = await axios.post(`${UPLOAD_BASE}/files?uploadType=multipart&supportsAllDrives=true`, formData, {
    headers: { ...h(token), "Content-Type": "multipart/related; boundary=boundary" }, timeout: 120000,
  });
...
async function opDownloadText(config, token) {
  ...
  const res = await axios.get(`${BASE}/files/${encodeURIComponent(config.fileId)}?alt=media&supportsAllDrives=true`, { headers: h(token), timeout: 120000, responseType: "text" });
...
async function opExportFile(config, token) {
  ...
  const res = await axios.get(`${BASE}/files/${encodeURIComponent(config.fileId)}/export`, {
    headers: h(token), timeout: 120000, responseType: "text",
...
async function opCopyFile(config, token) {
  ...
  const res = await axios.post(`${BASE}/files/${encodeURIComponent(config.fileId)}/copy`, body, {
    headers: h(token, true), timeout: 120000, params: { fields: FILE_FIELDS, ...SUPPORTS_ALL },
  });
  return slimFile(res.data);
}
...
async function opUpdateFileContent(config, token) {
  ...
  const res = await axios.patch(`${UPLOAD_BASE}/files/${encodeURIComponent(config.fileId)}?uploadType=media&supportsAllDrives=true`, config.content, {
    headers: { ...h(token), "Content-Type": mimeType }, timeout: 120000,
  });
...
async function opMoveFile(config, token) {
  ...
  const meta = await axios.get(`${BASE}/files/${encodeURIComponent(config.fileId)}`, { headers: h(token), params: { fields: "parents", ...SUPPORTS_ALL }, timeout: 120000 });
  const oldParents = (meta.data.parents ?? []).join(",");
  const res = await axios.patch(`${BASE}/files/${encodeURIComponent(config.fileId)}`, {}, { headers: h(token), params: { addParents: config.targetFolderId, removeParents: oldParents, fields: "id,parents", ...SUPPORTS_ALL }, timeout: 120000 });
...
async function opDeleteFile(config, token) {
  ...
  await axios.delete(`${BASE}/files/${encodeURIComponent(config.fileId)}`, { headers: h(token), timeout: 120000, params: SUPPORTS_ALL });
...
async function opTrashFile(config, token) {
  ...
  const res = await axios.patch(`${BASE}/files/${encodeURIComponent(config.fileId)}`, { trashed: true }, { headers: h(token, true), timeout: 120000, params: { fields: "id,trashed", ...SUPPORTS_ALL } });
...
async function opRestoreFile(config, token) {
  ...
  const res = await axios.patch(`${BASE}/files/${encodeURIComponent(config.fileId)}`, { trashed: false }, { headers: h(token, true), timeout: 120000, params: { fields: "id,trashed", ...SUPPORTS_ALL } });
  return { restored: true, fileId: res.data.id };
}

async function opEmptyTrash(config, token) {
  await axios.delete(`${BASE}/files/trash`, { headers: h(token), timeout: 120000 });
...
async function opStarFile(config, token) {
  ...
  const res = await axios.patch(`${BASE}/files/${encodeURIComponent(config.fileId)}`, { starred }, { headers: h(token, true), timeout: 120000, params: { fields: "id,starred", ...SUPPORTS_ALL } });
