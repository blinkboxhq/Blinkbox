...
async function opGetPR(config, token) {
  ...
  const res = await axios.get(`${repoPath(config)}/pulls/${encodeURIComponent(config.prNumber)}`, { headers: headers(token), timeout: 120000 });
...
async function opUpdatePR(config, token) {
  ...
  const res = await axios.patch(`${repoPath(config)}/pulls/${encodeURIComponent(config.prNumber)}`, body, { headers: headers(token), timeout: 120000 });
...
async function opListPRFiles(config, token) {
  ...
  const res = await axios.get(`${repoPath(config)}/pulls/${encodeURIComponent(config.prNumber)}/files`, { headers: headers(token), timeout: 120000, params: { per_page: PER_PAGE(config, 100) } });
...
async function opRequestReviewers(config, token) {
  ...
  const res = await axios.post(`${repoPath(config)}/pulls/${encodeURIComponent(config.prNumber)}/requested_reviewers`, { reviewers: csv(config.reviewers), team_reviewers: csv(config.teamReviewers) }, { headers: headers(token), timeout: 120000 });
