...
export default function NetlifyTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Netlify" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Netlify</span>
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                In Netlify → Site settings → Build hooks & notifications, add your BlinkBox URL as an outgoing webhook.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Site ID</label>
              <input value={config.siteId || ''} onChange={e => updateConfig?.('siteId', e.target.value)}
                placeholder="netlify site ID" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
          <div className="flex flex-col gap-1.5">
            ...
            <select value={config.event || 'all'} onChange={e => updateConfig?.('event', e.target.value)}
              ...
              <option value="deploy_succeeded">Deploy succeeded</option>
              <option value="deploy_failed">Deploy failed</option>
              <option value="deploy_started">Deploy started</option>
            </select>
          </div>
        )}
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.event','Event type'],['$trigger.siteId','Site ID'],['$trigger.siteName','Site name'],['$trigger.deployId','Deploy ID'],['$trigger.deployUrl','Deploy preview URL'],['$trigger.branch','Git branch'],['$trigger.deployTime','Build duration (seconds)']].map(([k,d]) => (
