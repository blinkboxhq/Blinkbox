            ...
            <div className="overflow-y-auto flex-1">
              ...
              {filtered.map((c) => (
                <button
                  ...
                  className="w-full px-3 py-2.5 text-left text-[13px] text-neutral-300 hover:bg-white/[0.06] transition-colors flex items-center gap-2.5"
                >
                  <Shield className={`w-3.5 h-3.5 shrink-0 ${c._id === value ? 'text-emerald-500' : 'text-neutral-600'}`} />
                  <span className="flex-1 truncate">{c.name}</span>
                  {c.type && <span className="text-[9px] font-bold text-neutral-700 uppercase">{c.type}</span>}
                  {c._id === value && <Check className="w-3.5 h-3.5 text-emerald-500 shrink-0" />}
                </button>
              ))}
            </div>
