    ...
    <div className="flex flex-col gap-5 w-full">
      ...
      {op === "setDocument" && (
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => updateConfig("merge", !config.merge)}
            className={`relative w-10 h-5 rounded-full transition-all duration-200 ${config.merge ? 'bg-orange-500' : 'bg-zinc-700'}`}
          >
            <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-200 ${config.merge ? 'left-5' : 'left-0.5'}`} />
          </button>
        </div>
      )}
