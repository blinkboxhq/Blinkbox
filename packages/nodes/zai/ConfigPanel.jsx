...
// Z.ai's native GLM family.
const MODELS_CHAT = ['glm-5.2', 'glm-5.1', 'glm-4.7', 'glm-4.5-air'];
const MODELS_CODE = ['glm-5.2', 'glm-4.7'];
const MODELS_REASON = ['glm-5.2', 'glm-5.1'];
const MODELS_VISION = ['glm-4.6v', 'glm-5.2'];
...
function ChatPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="zai" credentialId={config.credentialId} value={config.model} fallback="glm-5.2" onChange={(v) => updateConfig('model', v)} models={MODELS_CHAT} accentColor={MONO} />
      ...
      <Text label="Prompt" value={config.prompt} onChange={(v) => updateConfig('prompt', v)} placeholder="Ask GLM anything…" multiline nodeId={nodeId} />
...
function CodePanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="zai" credentialId={config.credentialId} value={config.model} fallback="glm-5.2" onChange={(v) => updateConfig('model', v)} models={MODELS_CODE} accentColor={MONO} />
...
function ReasoningPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="zai" credentialId={config.credentialId} value={config.model} fallback="glm-5.2" onChange={(v) => updateConfig('model', v)} models={MODELS_REASON} accentColor={MONO} />
...
function VisionPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="zai" credentialId={config.credentialId} value={config.model} fallback="glm-4.6v" onChange={(v) => updateConfig('model', v)} models={MODELS_VISION} accentColor={MONO} />
...
const ACTIONS = {
  'Chat Completion':         { op: 'message', desc: "Chat with GLM-5.2 and the GLM family",          Panel: ChatPanel },
  'Code Generation':         { op: 'code', desc: "Generate or explain code with GLM",             Panel: CodePanel },
  'Structured Output':       { op: 'structuredOutput', desc: "Return JSON matching a schema", Panel: StructuredPanel },
  'Function Calling':        { op: 'functionCalling', desc: "Let GLM call your tools/functions",  Panel: FunctionPanel },
  'Reasoning':               { op: 'reasoning', desc: "Deep step-by-step reasoning with GLM-5.2",        Panel: ReasoningPanel },
...
export default function ZaiNode({ config = {}, updateConfig, nodeId }) {
  ...
  return (
    <ConfigSection>
      ...
      <CredentialPicker
        ...
        accentColor="blue"
        label="Z.ai API Key"
        placeholder="Select Z.ai credential…"
      />
    </ConfigSection>
  );
}
