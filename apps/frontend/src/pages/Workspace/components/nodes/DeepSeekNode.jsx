import {
  MessageSquare, Brain, PenLine, Sparkles, ChevronDown,
} from 'lucide-react';
...
const OPERATIONS = [
  {
    ...
    icon: MessageSquare,
    description: 'Chat with deepseek-chat — text or JSON output',
  },
  {
    value: 'reasoningMessage',
    label: 'Reasoning message',
    icon: Brain,
    description: 'Deep chain-of-thought with deepseek-reasoner (returns reasoning + result)',
  },
  {
  ...
  return (
    <div className="relative">
      <select
        ...
        className="w-full bg-[#111] border border-[#333] rounded-lg px-3 py-2 pr-8 text-xs text-white font-semibold focus:outline-none focus:border-cyan-500/50 transition-colors cursor-pointer appearance-none"
      >
        {options.map((o) => (
      ...
      <div className="grid grid-cols-1 gap-1">
        {OPERATIONS.map((op) => {
          ...
          return (
            ...
            >
              <Icon className={`w-4 h-4 shrink-0 ${active ? 'text-cyan-400' : 'text-zinc-500'}`} strokeWidth={1.75} />
              <div className="flex-1 min-w-0">
                <div className={`text-xs font-semibold leading-tight ${active ? 'text-cyan-300' : 'text-zinc-300'}`}>{op.label}</div>
...
function MessageFields({ config, updateConfig }) {
  return (
    <>
      <Field label="Model">
        <Select value={config.model || 'deepseek-chat'} onChange={(v) => updateConfig('model', v)} options={MODELS_CHAT} />
      </Field>
      ...
      <Field label="Prompt">
        <SmartVariableInput value={config.prompt || ''} onChange={(v) => updateConfig('prompt', v)} placeholder="e.g. Reason through this problem step by step..." multiline />
      </Field>
    </>
  );
}

function ReasoningMessageFields({ config, updateConfig }) {
  return (
    <>
      <Field
        label="Prompt"
        hint="Uses deepseek-reasoner automatically. Output includes both 'result' and 'reasoning' fields."
      >
        <SmartVariableInput value={config.prompt || ''} onChange={(v) => updateConfig('prompt', v)} placeholder="e.g. Solve this math problem and show all steps..." multiline />
      </Field>
    </>
  );
}
...
function GeneratePromptFields({ config, updateConfig }) {
  return (
    <Field label="Task description">
      <SmartVariableInput value={config.task || ''} onChange={(v) => updateConfig('task', v)} placeholder="e.g. Summarize a legal document into plain English..." multiline />
    </Field>
  );
}
...
const OP_FIELDS = {
  ...
  reasoningMessage: ReasoningMessageFields,
