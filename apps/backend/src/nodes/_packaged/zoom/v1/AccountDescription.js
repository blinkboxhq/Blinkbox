...
async function opListRecordings(config, token) {
  ...
  const res = await axios.get(`${BASE}/users/${encodeURIComponent(userId)}/recordings?${params.toString()}`, { headers: headers(token), timeout: 15000 });
...
async function opGetRecording(config, token) {
  ...
  const res = await axios.get(`${BASE}/meetings/${encodeURIComponent(config.meetingId)}/recordings`, { headers: headers(token), timeout: 15000 });
...
async function opDeleteRecording(config, token) {
  ...
  await axios.delete(`${BASE}/meetings/${encodeURIComponent(config.meetingId)}/recordings?action=${config.trash === true ? "trash" : "delete"}`, { headers: headers(token), timeout: 10000 });
...
async function opCreateWebinar(config, token) {
  ...
  const userId = config.userId || "me";
  const res = await axios.post(`${BASE}/users/${encodeURIComponent(userId)}/webinars`, body, { headers: headers(token), timeout: 15000 });
  const w = res.data;
...
async function opListWebinars(config, token) {
  const userId = config.userId || "me";
  const res = await axios.get(`${BASE}/users/${encodeURIComponent(userId)}/webinars?page_size=30`, { headers: headers(token), timeout: 10000 });
  return { webinars: res.data.webinars || [], total: res.data.total_records };
}

async function opGetUser(config, token) {
  const userId = config.userId || "me";
  const res = await axios.get(`${BASE}/users/${encodeURIComponent(userId)}`, { headers: headers(token), timeout: 10000 });
  const u = res.data;
...
async function opListUsers(config, token) {
  ...
  const res = await axios.get(`${BASE}/users?status=${encodeURIComponent(status)}&page_size=30`, { headers: headers(token), timeout: 10000 });
