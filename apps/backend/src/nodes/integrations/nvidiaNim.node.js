 ...
 *   chat completions  ✓   vision (VLM image_url)  ✓   embeddings  ✓
 *   tool/function calling ✓ (on models that support it)
 *   image generation / audio  ✗ (not on the OpenAI-compatible LLM endpoint)
 *
 * Config:
 *   operation    — see OPERATIONS below (default: "message")
 *   model        — NIM model id (default per operation)
 *   prompt       — instruction / user message
 *   credentialId — Vault reference to the nvapi- API key (type "NvidiaNim")
 */
...
const DEFAULT_CODE_MODEL  = "qwen/qwen3-coder-480b-a35b-instruct";
...
async function opAnalyzeImage(config, input, apiKey) {
  ...
  if (!imageUrl) return { success: false, error: "NVIDIA NIM analyzeImage: 'imageUrl' is required.", skipped: true };
  ...
  return { result: data.choices?.[0]?.message?.content || "", model: data.model || model, tokensUsed: data.usage?.total_tokens || 0, provider: "nvidiaNim", operation: "analyzeImage" };
}
...
// Backwards-compat aliases for older saved workflows.
...
export default {
  async run(config, input, context = {}) {
    ...
    if (!handler) throw new Error(`NVIDIA NIM: Unknown operation "${operation}". Valid: ${Object.keys(OPERATIONS).join(", ")}`);
