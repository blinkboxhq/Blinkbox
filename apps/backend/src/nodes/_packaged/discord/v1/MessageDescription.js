 ...
 */
import { bot, need, buildEmbed, p } from "../GenericFunctions.js";
...
async function opBotSendMessage(config, token) {
  ...
  const data = await bot(token, "post", p`/channels/${config.channelId}/messages`, body);
...
async function opEditMessage(config, token) {
  ...
  const data = await bot(token, "patch", p`/channels/${config.channelId}/messages/${config.messageId}`, body);
...
async function opDeleteMessage(config, token) {
  ...
  await bot(token, "delete", p`/channels/${config.channelId}/messages/${config.messageId}`);
...
async function opGetMessages(config, token) {
  ...
  const data = await bot(token, "get", p`/channels/${config.channelId}/messages`, null, { limit: Math.min(Number(config.limit) || 50, 100) });
...
async function opPinMessage(config, token) {
  ...
  await bot(token, "put", p`/channels/${config.channelId}/pins/${config.messageId}`);
...
async function opUnpinMessage(config, token) {
  ...
  await bot(token, "delete", p`/channels/${config.channelId}/pins/${config.messageId}`);
...
async function opAddReaction(config, token) {
  ...
  const emoji = String(config.emoji).replace(/:/g, "");
  await bot(token, "put", p`/channels/${config.channelId}/messages/${config.messageId}/reactions/${emoji}/@me`);
...
async function opRemoveReaction(config, token) {
  ...
  const emoji = String(config.emoji).replace(/:/g, "");
  await bot(token, "delete", p`/channels/${config.channelId}/messages/${config.messageId}/reactions/${emoji}/@me`);
...
async function opCreateThread(config, token) {
  ...
  const path = config.messageId
    ? p`/channels/${config.channelId}/messages/${config.messageId}/threads`
    : p`/channels/${config.channelId}/threads`;
