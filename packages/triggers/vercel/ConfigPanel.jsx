...
export default function VercelTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Vercel" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Vercel</span>
        <div className="ml-auto">
          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded border"
            style={{ color: accent, background: '#ffffff18', borderColor: '#ffffff33' }}>TRIGGER</span>
        </div>
      </div>
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                In Vercel → Project Settings → Webhooks, add your BlinkBox URL.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Project Name</label>
              <input value={config.projectName || ''} onChange={e => updateConfig?.('projectName', e.target.value)}
                placeholder="my-vercel-project" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
          <div className="flex flex-col gap-1.5">
            ...
            <select value={config.event || 'all'} onChange={e => updateConfig?.('event', e.target.value)}
              ...
              <option value="deployment.created">Deployment created</option>
              <option value="deployment.succeeded">Deployment succeeded</option>
              <option value="deployment.error">Deployment error</option>
              <option value="deployment.canceled">Deployment canceled</option>
            </select>
          </div>
        )}
        {tab === 'payload' && (
