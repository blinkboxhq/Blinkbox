...
import { Network, Activity, Key, Settings, LogOut, LayoutTemplate, ChevronLeft, ChevronRight, Zap } from 'lucide-react';
...
export default function DashboardSidebar({ user, onLogout, activeTab, setActiveTab, usage }) {
  ...
  return (
    <>
      <aside className={`${w} bg-[#0a0a0a] border-r border-neutral-900/80 flex flex-col shrink-0 relative z-20 transition-all duration-200`}>
        {/* Header */}
        ...
        <div className={`border-t border-neutral-900/80 ${expanded ? 'p-3' : 'p-2'}`}>
          {expanded ? (
            <div className="flex items-center gap-2.5 px-1">
              <div className="w-7 h-7 rounded-full bg-neutral-800 flex items-center justify-center text-[11px] font-semibold text-neutral-400 uppercase shrink-0">
                {user?.name?.charAt(0) || '?'}
              </div>
            ...
            <button
              ...
              title="Log out"
            >
              <div className="w-7 h-7 rounded-full bg-neutral-800 flex items-center justify-center text-[11px] font-semibold text-neutral-400 uppercase hover:bg-neutral-700 transition-colors">
                {user?.name?.charAt(0) || '?'}
              </div>
            </button>
          )}
      ...
      {showLogout && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[2px]" style={{ animation: 'dbFadeIn 0.12s ease-out' }}>
          <div className="bg-[#111] border border-neutral-800 rounded-xl w-full max-w-[340px] p-5 mx-4" style={{ animation: 'dbScaleIn 0.12s ease-out' }}>
