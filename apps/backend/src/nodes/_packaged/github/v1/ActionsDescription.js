...
async function opListReleases(config, token) {
  const res = await axios.get(`${repoPath(config)}/releases`, { headers: headers(token), timeout: 120000, params: { per_page: PER_PAGE(config) } });
...
async function opGetLatestRelease(config, token) {
  const res = await axios.get(`${repoPath(config)}/releases/latest`, { headers: headers(token), timeout: 120000 });
...
async function opListWorkflowRuns(config, token) {
  ...
  const res = await axios.get(url, { headers: headers(token), timeout: 120000, params: { per_page: PER_PAGE(config), branch: config.branch || undefined, status: config.runStatus || undefined } });
...
async function opDispatchWorkflow(config, token) {
  ...
  await axios.post(`${repoPath(config)}/actions/workflows/${encodeURIComponent(config.workflowId)}/dispatches`, { ref: config.branch || "main", inputs }, { headers: headers(token), timeout: 120000 });
