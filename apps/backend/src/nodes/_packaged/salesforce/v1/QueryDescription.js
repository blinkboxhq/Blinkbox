...
async function opQueryRecords(config, client) {
  ...
  const { data } = await client.get(`/query`, { timeout: 120000, params: { q: config.query } });
...
async function opSearchRecords(config, client) {
  ...
  const { data } = await client.get(`/search`, { timeout: 120000, params: { q: config.query } });
...
async function opQueryAll(config, client) {
  ...
  const { data } = await client.get(`/queryAll`, { timeout: 120000, params: { q: config.query } });
  return { success: true, records: data.records ?? [], totalSize: data.totalSize, done: data.done, nextRecordsUrl: data.nextRecordsUrl };
}
...
async function opQueryMore(config, client) {
  ...
  const { data } = await client.get(`/query/${client.enc(locator)}`, { timeout: 120000 });
  return { success: true, records: data.records ?? [], totalSize: data.totalSize, done: data.done, nextRecordsUrl: data.nextRecordsUrl };
}

async function opGetLimits(_config, client) {
  const { data } = await client.get(`/limits`, { timeout: 120000 });
