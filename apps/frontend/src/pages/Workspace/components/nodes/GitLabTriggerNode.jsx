...
export default function GitLabTriggerNode({ config = {}, updateConfig, selected }) {
  const eventType = config.eventType || 'merge_request';
  return (
    <div className={`relative flex flex-col w-[280px] bg-[#0A0A0A] rounded-xl border shadow-2xl font-sans group ${selected ? 'border-[#FC6D26]/50' : 'border-[#2A2A2A]'}`}>
      <Handle id="output" type="source" position={Position.Right}
        className="!w-5 !h-5 !flex items-center justify-center !bg-[#111] !border !border-[#2A2A2A] !rounded-full !opacity-0 group-hover:!opacity-100 transition-all hover:!bg-[#FC6D26] hover:!border-[#FC6D26] z-50 cursor-crosshair"
