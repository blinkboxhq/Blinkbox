...
const API_KEY_NODES = new Set([
  ...
  'stripe', 'shopify', 'hubspot', 'notion', 'github', 'jira', 'linear',
  'mongodb', 'postgres', 'redis', 'firebase', 'supabase', 'pinecone',
  'elevenlabs', 'twitter', 'web_search', 'zoom', 'notion_trigger',
...
function OAuthPrompt({ provider, label, onConnected }) {
  ...
  const connect = useCallback(() => {
    ...
    const poll = setInterval(() => { if (popup?.closed) { clearInterval(poll); setConnecting(false); window.removeEventListener('message', handler); } }, 500);
...
// ── Main plan card ────────────────────────────────────────────────────────────
export default function BrianWorkflowPlan({ text, flow, onAccept, onDismiss }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Plan card */}
      <div className="border border-[#2a2a2a] rounded-xl overflow-hidden bg-neutral-900/40">
        {/* Header */}
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2a2a2a] bg-neutral-900/60">
          <div className="w-1.5 h-1.5 rounded-full bg-violet-500" />
          <span className="text-[10px] font-semibold text-neutral-400 uppercase tracking-widest">Workflow Plan</span>
          <span className="ml-auto text-[10px] font-mono text-neutral-600">{nodes.length} nodes · {edges.length} edges</span>
        </div>
        ...
        {/* Actions */}
        {!accepted ? (
          <div className="flex items-center gap-2 px-3 py-2.5 border-t border-[#2a2a2a] bg-neutral-900/30">
