...
async function opAddComment(config, ctx) {
  ...
  const res = await axios.post(`${ctx.BASE}/issue/${encodeURIComponent(config.issueKey)}/comment`, { body: adf(config.comment) }, { headers: ctx.headers, timeout: 120000 });
...
async function opGetComments(config, ctx) {
  ...
  const res = await axios.get(`${ctx.BASE}/issue/${encodeURIComponent(config.issueKey)}/comment`, { headers: ctx.headers, timeout: 120000, params: { maxResults: LIMIT(config) } });
...
async function opUpdateComment(config, ctx) {
  ...
  const res = await axios.put(`${ctx.BASE}/issue/${encodeURIComponent(config.issueKey)}/comment/${encodeURIComponent(config.commentId)}`, { body: adf(config.comment) }, { headers: ctx.headers, timeout: 120000 });
...
async function opDeleteComment(config, ctx) {
  ...
  await axios.delete(`${ctx.BASE}/issue/${encodeURIComponent(config.issueKey)}/comment/${encodeURIComponent(config.commentId)}`, { headers: ctx.headers, timeout: 120000 });
