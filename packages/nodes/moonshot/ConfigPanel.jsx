...
const MODELS_CHAT = ['kimi-k3', 'kimi-k2.6', 'kimi-k2.5', 'kimi-k2-instruct', 'moonshot-v1-128k', 'moonshot-v1-32k'];
...
function ChatPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="moonshot" credentialId={config.credentialId} value={config.model} fallback="kimi-k2.6" onChange={(v) => updateConfig('model', v)} models={MODELS_CHAT} accentColor={MONO} />
...
function CodePanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="moonshot" credentialId={config.credentialId} value={config.model} fallback="kimi-k2.7-code" onChange={(v) => updateConfig('model', v)} models={MODELS_CODE} accentColor={MONO} />
...
function ReasoningPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="moonshot" credentialId={config.credentialId} value={config.model} fallback="kimi-k2-thinking" onChange={(v) => updateConfig('model', v)} models={MODELS_THINK} accentColor={MONO} />
...
function VisionPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="moonshot" credentialId={config.credentialId} value={config.model} fallback="kimi-k2.6" onChange={(v) => updateConfig('model', v)} models={MODELS_VISION} accentColor={MONO} />
...
const ACTIONS = {
  'Chat Completion':         { op: 'message', desc: "Query Kimi for a conversational response",          Panel: ChatPanel },
  'Code Generation':         { op: 'code', desc: "Generate or explain code with Kimi-Code",             Panel: CodePanel },
  'Structured Output':       { op: 'structuredOutput', desc: "Return JSON matching a schema", Panel: StructuredPanel },
  'Function Calling':        { op: 'functionCalling', desc: "Let Kimi call your tools/functions",  Panel: FunctionPanel },
  'Reasoning':               { op: 'reasoning', desc: "Thinking mode for hard, multi-step problems",        Panel: ReasoningPanel },
