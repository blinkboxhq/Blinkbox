...
async function opListBranches(config, token) {
  const res = await axios.get(`${repoPath(config)}/branches`, { headers: headers(token), timeout: 120000, params: { per_page: PER_PAGE(config, 100) } });
...
async function opGetBranch(config, token) {
  ...
  const res = await axios.get(`${repoPath(config)}/branches/${encodeURIComponent(config.branch)}`, { headers: headers(token), timeout: 120000 });
...
async function opCreateBranch(config, token) {
  ...
  const baseRef = await axios.get(`${repoPath(config)}/git/ref/heads/${encodeURIComponent(fromRef)}`, { headers: headers(token), timeout: 120000 });
  const res = await axios.post(`${repoPath(config)}/git/refs`, { ref: `refs/heads/${config.branch}`, sha: baseRef.data.object.sha }, { headers: headers(token), timeout: 120000 });
...
async function opGetCommit(config, token) {
  ...
  const res = await axios.get(`${repoPath(config)}/commits/${encodeURIComponent(config.sha)}`, { headers: headers(token), timeout: 120000 });
