...
import { Plus, Mail, Copy, Check, Lock, ChevronDown } from 'lucide-react';
...
export default function EmailTriggerNode({ config = {}, updateConfig, selected }) {
  ...
  return (
    <div className={`relative flex flex-col w-[280px] bg-[#0A0A0A] rounded-xl border transition-colors shadow-2xl font-sans group ${selected ? 'border-violet-500/50' : 'border-[#2A2A2A]'}`}>
      ...
      <div className="flex bg-[#0a0a0a] px-3 pt-2 gap-3 border-b border-[#1a1a1a]">
        {['setup', 'payload', 'security'].map((tab) => (
          <button
            key={tab}
      ...
      <div className="p-3 flex flex-col gap-3">
        ...
        {activeTab === 'payload' && (
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Available in workflow as</label>
              <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
                {[
                  ['$trigger.body.from', 'Sender email address'],
                  ['$trigger.body.subject', 'Email subject line'],
                  ['$trigger.body.text', 'Plain text body'],
                  ['$trigger.body.html', 'HTML body'],
                  ['$trigger.body.to', 'Recipient address'],
                  ['$trigger.body.attachments', 'Array of attachments'],
                ].map(([key, desc]) => (
            ...
            <p className="text-[9px] text-zinc-600 leading-relaxed">
              Field names may vary slightly by provider. Check your provider's inbound parse docs if a field is missing.
            </p>
          </div>
        )}
