...
async function opCreateDomain(config, apiKey) {
  ...
  const res = await axios.post(`${BASE}/domains`, body, { headers: headers(apiKey), timeout: 120000 });
  return res.data;
}
...
async function opGetDomain(config, apiKey) {
  ...
  const res = await axios.get(`${BASE}/domains/${encodeURIComponent(config.domainId)}`, { headers: headers(apiKey), timeout: 120000 });
  return res.data;
}

async function opListDomains(config, apiKey) {
  const res = await axios.get(`${BASE}/domains`, { headers: headers(apiKey), timeout: 120000 });
...
async function opVerifyDomain(config, apiKey) {
  ...
  const res = await axios.post(`${BASE}/domains/${encodeURIComponent(config.domainId)}/verify`, {}, { headers: headers(apiKey), timeout: 120000 });
...
async function opUpdateDomain(config, apiKey) {
  ...
  const res = await axios.patch(`${BASE}/domains/${encodeURIComponent(config.domainId)}`, body, { headers: headers(apiKey), timeout: 120000 });
...
async function opDeleteDomain(config, apiKey) {
  ...
  const res = await axios.delete(`${BASE}/domains/${encodeURIComponent(config.domainId)}`, { headers: headers(apiKey), timeout: 120000 });
...
async function opCreateApiKey(config, apiKey) {
  ...
  const res = await axios.post(`${BASE}/api-keys`, body, { headers: headers(apiKey), timeout: 120000 });
  return res.data;
}

async function opListApiKeys(config, apiKey) {
  const res = await axios.get(`${BASE}/api-keys`, { headers: headers(apiKey), timeout: 120000 });
  return { data: res.data.data ?? [] };
}
...
async function opDeleteApiKey(config, apiKey) {
  ...
  await axios.delete(`${BASE}/api-keys/${encodeURIComponent(config.apiKeyId)}`, { headers: headers(apiKey), timeout: 120000 });
