  ...
  async run(config, input, context = {}) {
    ...
    try {
      ...
      if (operation === "getDocument") {
        if (!collection || !docId) throw new Error("Firebase: 'collection' and 'docId' are required.");
      ...
      if (operation === "addDocument") {
        if (!collection) throw new Error("Firebase: 'collection' is required.");
        const payload = parseJson(data, "data");
      ...
      if (operation === "getUser") {
        if (!userId && !email) throw new Error("Firebase: 'userId' or 'email' is required.");
      ...
      if (operation === "sendNotification") {
        if (!fcmToken) throw new Error("Firebase: 'fcmToken' is required.");
