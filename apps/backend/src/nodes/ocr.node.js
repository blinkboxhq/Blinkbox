  ...
  async run(config, input, context = {}) {
    ...
    if (provider === "anthropic") {
      ...
      const { data } = await axios.post("https://api.anthropic.com/v1/messages",
        { model: config.model || "claude-haiku-4-5", max_tokens: 2048, messages: [{ role: "user", content: [{ type: "image", source: { type: "base64", media_type: mediaType, data: b64 } }, { type: "text", text: prompt }] }] },
        { headers: { "x-api-key": apiKey, "anthropic-version": "2023-06-01", "Content-Type": "application/json" }, timeout: 60000 }
      );
      return { text: data.content[0].text, provider: "anthropic", model: config.model || "claude-haiku-4-5" };
    }
