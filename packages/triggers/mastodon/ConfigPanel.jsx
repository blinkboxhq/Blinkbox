...
export default function MastodonTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Mastodon" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Mastodon</span>
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                Mastodon supports webhooks via the admin API. Add your BlinkBox URL in Preferences → Development.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Instance URL</label>
              <input value={config.instanceUrl || ''} onChange={e => updateConfig?.('instanceUrl', e.target.value)}
                placeholder="https://mastodon.social" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Access Token</label>
              <input value={config.accessToken || ''} onChange={e => updateConfig?.('accessToken', e.target.value)}
                placeholder="your access token" type="password" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
          <div className="flex flex-col gap-1.5">
            ...
            <select value={config.event || 'all'} onChange={e => updateConfig?.('event', e.target.value)}
              ...
              <option value="status">New status</option>
        ...
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.event','Event type'],['$trigger.accountId','Account ID'],['$trigger.accountUsername','Account username'],['$trigger.content','Status content'],['$trigger.statusId','Status ID'],['$trigger.createdAt','When event occurred']].map(([k,d]) => (
