  ...
  return (
    <div className="flex flex-col gap-5 w-full">
      ...
      <div className="flex flex-col gap-2">
        ...
        <SmartVariableInput nodeId={nodeId} value={config.shop || ""} onChange={(v) => updateConfig("shop", v)} placeholder="mystore.myshopify.com" />
      </div>
      ...
      {["getProduct", "updateProduct"].includes(op) && (
        <div className="flex flex-col gap-2">
          ...
          <SmartVariableInput nodeId={nodeId} value={config.productId || ""} onChange={(v) => updateConfig("productId", v)} placeholder="{{n1.id}}" />
        </div>
      )}
      ...
      {["createProduct", "updateProduct"].includes(op) && (
        <>
          <div className="flex flex-col gap-2">
            ...
            <SmartVariableInput nodeId={nodeId} value={config.title || ""} onChange={(v) => updateConfig("title", v)} placeholder="Premium Widget" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Description (HTML)</label>
            <SmartVariableInput nodeId={nodeId} value={config.description || ""} onChange={(v) => updateConfig("description", v)} placeholder="<p>Best widget ever</p>" multiline />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Price</label>
              <SmartVariableInput nodeId={nodeId} value={config.price || ""} onChange={(v) => updateConfig("price", v)} placeholder="29.99" />
            </div>
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-1.5">
                {["draft", "active", "archived"].map((s) => (
                  <button key={s} onClick={() => updateConfig("status", s)}
                    className={`flex-1 py-2 rounded-lg border text-xs font-bold capitalize transition-all ${(config.status || "draft") === s ? "bg-[#95BF47]/10 border-[#95BF47]/40 text-[#95BF47]" : "bg-[#0a0a0a] border-[#222] text-zinc-500 hover:border-[#333]"}`}>
                    {s}
                  </button>
                ))}
              </div>
      ...
      {["getOrder", "updateOrder"].includes(op) && (
        <div className="flex flex-col gap-2">
          ...
          <SmartVariableInput nodeId={nodeId} value={config.orderId || ""} onChange={(v) => updateConfig("orderId", v)} placeholder="{{n1.id}}" />
        </div>
      )}
