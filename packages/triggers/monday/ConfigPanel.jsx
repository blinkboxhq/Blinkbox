...
export default function MondayTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Monday.com" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Monday.com</span>
      ...
      <div className="p-3 flex flex-col gap-3">
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                In Monday.com → Integrations → Webhooks, add your BlinkBox URL.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Board ID</label>
              <input value={config.boardId || ''} onChange={e => updateConfig?.('boardId', e.target.value)}
                placeholder="board ID" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
          <div className="flex flex-col gap-1.5">
            ...
            <select value={config.event || 'all'} onChange={e => updateConfig?.('event', e.target.value)}
              ...
              <option value="create_item">Item created</option>
              <option value="update_item">Item updated</option>
              <option value="create_subitem">Subitem created</option>
        ...
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.event','Event type'],['$trigger.boardId','Board ID'],['$trigger.itemId','Item ID'],['$trigger.itemName','Item name'],['$trigger.columnId','Column that changed'],['$trigger.previousValue','Previous column value'],['$trigger.value','New column value']].map(([k,d]) => (
