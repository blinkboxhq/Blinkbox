 ...
 * OPENROUTER NODE
 *
 * OpenAI-compatible. Routes to 100+ models via OpenRouter.
 *
 * Output: { result, model, tokensUsed, provider: "openrouter" }
 */
...
export default {
  async run(config, input, context = {}) {
    ...
    const cred = await resolveCredential(credentialId, context.workspaceId, "OpenRouter");
    ...
    try {
      ...
      if (err.response?.status === 429) throw new Error("OpenRouter: Rate limit exceeded. Retry later.");
      throw new Error(`OpenRouter failed: ${err.response?.status || err.code} — ${err.message}`);
    }
  },
};
