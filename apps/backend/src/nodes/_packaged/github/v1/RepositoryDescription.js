...
async function opGetRepo(config, token) {
  const res = await axios.get(repoPath(config), { headers: headers(token), timeout: 120000 });
...
async function opListMyRepos(config, token) {
  const res = await axios.get(`${BASE}/user/repos`, { headers: headers(token), timeout: 120000, params: { per_page: PER_PAGE(config), sort: config.sort || "updated", visibility: config.visibility || "all", affiliation: config.affiliation || undefined } });
...
async function opGetUser(config, token) {
  ...
  const res = await axios.get(`${BASE}/users/${encodeURIComponent(config.username)}`, { headers: headers(token), timeout: 120000 });
...
async function opGetAuthenticatedUser(config, token) {
  const res = await axios.get(`${BASE}/user`, { headers: headers(token), timeout: 120000 });
...
async function opSearchIssues(config, token) {
  ...
  const res = await axios.get(`${BASE}/search/issues`, { headers: headers(token), timeout: 120000, params: { q: config.query, per_page: PER_PAGE(config), sort: config.sort || undefined } });
...
async function opSearchRepos(config, token) {
  ...
  const res = await axios.get(`${BASE}/search/repositories`, { headers: headers(token), timeout: 120000, params: { q: config.query, per_page: PER_PAGE(config), sort: config.sort || undefined } });
...
async function opSearchCode(config, token) {
  ...
  const res = await axios.get(`${BASE}/search/code`, { headers: headers(token), timeout: 120000, params: { q: config.query, per_page: PER_PAGE(config) } });
