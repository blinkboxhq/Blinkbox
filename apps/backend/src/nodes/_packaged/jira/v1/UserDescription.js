...
async function opGetCurrentUser(config, ctx) {
  const res = await axios.get(`${ctx.BASE}/myself`, { headers: ctx.headers, timeout: 120000 });
...
async function opSearchUsers(config, ctx) {
  ...
  const res = await axios.get(`${ctx.BASE}/user/search`, { headers: ctx.headers, timeout: 120000, params: { query: config.query, maxResults: LIMIT(config) } });
