...
// Proof-of-ownership secret, generated by the installer and shared with the
// cloud once over the registration call. The cloud asks /health for a
// fingerprint of it before pointing a blinkbox.net name at an address, so a
// caller cannot claim a subdomain for a machine they do not run.
export const SELF_HOST_PROBE_TOKEN = process.env.SELF_HOST_PROBE_TOKEN || null;

// The box's own globally-routable addresses as the installer found them. The
// egress IP a service like ipify reports is often *not* the inbound one on a
// NAT'd or multi-homed VPS, which is exactly how DNS ends up on a stranger's
// address, so these are reported alongside it and the cloud probes to decide.
export const SELF_HOST_PUBLIC_IPS = String(process.env.SELF_HOST_PUBLIC_IPS || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean)
  .slice(0, 8);
