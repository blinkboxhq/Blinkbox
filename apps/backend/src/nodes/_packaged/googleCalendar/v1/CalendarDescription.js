...
async function opGetCalendar(config, token) {
  const res = await axios.get(`${BASE}/calendars/${cal(config)}`, { headers: authHeaders(token), timeout: 120000 });
...
async function opCreateCalendar(config, token) {
  ...
  const res = await axios.post(`${BASE}/calendars`, body, { headers: authHeaders(token, true), timeout: 120000 });
...
async function opUpdateCalendar(config, token) {
  ...
  const res = await axios.patch(`${BASE}/calendars/${cal(config)}`, patch, { headers: authHeaders(token, true), timeout: 120000 });
...
async function opDeleteCalendar(config, token) {
  ...
  await axios.delete(`${BASE}/calendars/${cal(config)}`, { headers: authHeaders(token), timeout: 120000 });
...
async function opClearCalendar(config, token) {
  ...
  await axios.post(`${BASE}/calendars/primary/clear`, null, { headers: authHeaders(token), timeout: 120000 });
...
async function opAddCalendarToList(config, token) {
  ...
  const res = await axios.post(`${BASE}/users/me/calendarList`, { id: config.calendarId, colorId: config.colorId || undefined }, { headers: authHeaders(token, true), timeout: 120000 });
...
async function opRemoveCalendarFromList(config, token) {
  ...
  await axios.delete(`${BASE}/users/me/calendarList/${encodeURIComponent(config.calendarId)}`, { headers: authHeaders(token), timeout: 120000 });
