...
export default function WeatherNode({ config = {}, updateConfig }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <div>
        <label className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider mb-1.5 block">OpenWeatherMap API Key</label>
        <input type="password" value={apiKey} onChange={(e) => updateConfig('apiKey', e.target.value)} placeholder="Get free key at openweathermap.org"
          className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-[13px] text-zinc-100 focus:outline-none focus:border-zinc-500" />
      </div>
