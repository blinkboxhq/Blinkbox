...
export default function MailchimpTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Mailchimp" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Mailchimp</span>
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                In Mailchimp → Audience → Manage contacts → Webhooks, add your BlinkBox URL.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">List ID</label>
              <input value={config.listId || ''} onChange={e => updateConfig?.('listId', e.target.value)}
                placeholder="audience list ID" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
        ...
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.type','Event type'],['$trigger.email','Subscriber email'],['$trigger.firstName','First name'],['$trigger.lastName','Last name'],['$trigger.listId','Audience list ID'],['$trigger.firedAt','When event fired']].map(([k,d]) => (
