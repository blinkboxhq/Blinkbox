    ...
    <div className="flex flex-col gap-5 w-full">
      ...
      <div className="flex items-center gap-3 p-4 bg-pink-500/10 border border-pink-500/20 rounded-xl relative overflow-hidden shadow-[0_0_20px_rgba(244,114,182,0.05)]">
        ...
        <div className="flex flex-col z-10">
          ...
          <span className="text-[10px] text-slate-400 truncate mt-0.5">Route data based on conditions</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Routing Paths</label>
        ...
        {routes.length === 0 ? (
          <div className="text-center py-6 border border-dashed border-[#222] rounded-xl text-xs text-slate-600 bg-[#0a0a0a]">
          ...
          <div className="flex flex-col gap-3">
            {routes.map((route, i) => (
              <div key={i} className="flex flex-col gap-2 p-3 bg-[#0a0a0a] border border-[#222] rounded-xl relative group">
                <button onClick={() => removeRoute(i)} className="absolute top-2 right-2 p-1 text-slate-600 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
                
                <div className="flex flex-col gap-1.5 pr-6">
                  <label className="text-[9px] font-bold text-slate-500 uppercase">Path Name (Edge Condition)</label>
                  <input 
                    value={route.path} 
      ...
      <div className="p-3 bg-pink-500/5 border border-pink-500/10 rounded-lg text-[10px] text-slate-500 leading-relaxed">
