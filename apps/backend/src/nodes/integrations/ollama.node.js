 ...
 * OLLAMA NODE
 *
 * OpenAI-compatible. Local inference via Ollama server.
 * No API key required — runs on localhost:11434.
 *
 * Output: { result, model, tokensUsed, provider: "ollama" }
 */
...
export default {
  async run(config, input, context = {}) {
    ...
    try {
      ...
      throw new Error(`Ollama failed: ${err.response?.status || err.code} — ${err.message}`);
    }
  },
};
