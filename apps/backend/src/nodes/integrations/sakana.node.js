 ...
 * call other models, deciding per request whether to answer alone or
 * orchestrate a team. OpenAI-SDK compatible at https://api.sakana.ai/v1.
 *   fugu        — standard work
 *   fugu-ultra  — complex, high-value tasks (pin: fugu-ultra-20260615)
 *
 * Genuine API surface (be honest):
 *   chat completions  ✓   tool/function calling ✓   JSON mode  ✓
 *   reasoning / multi-agent orchestration  ✓ (the model routes internally)
 *   vision is delegated to downstream models when Fugu chooses to — pass an
 *   image and the orchestrator handles it, or errors honestly if it can't.
 *   image / audio generation  ✗ (route those to their native node)
 *
 * Config:
 *   operation    — see OPERATIONS below (default: "message")
 *   model        — "fugu" | "fugu-ultra" | pinned variant (default per op)
 *   prompt       — instruction / user message
 *   credentialId — Vault reference to the Sakana API key (type "Sakana")
 */

import axios from "axios";
import { getOAuthToken } from "../../utils/getOAuthToken.js";

const BASE = "https://api.sakana.ai/v1";

const DEFAULT_CHAT_MODEL   = "fugu";
const DEFAULT_VISION_MODEL = "fugu-ultra";
const DEFAULT_THINK_MODEL  = "fugu-ultra";
const DEFAULT_CODE_MODEL   = "fugu-ultra";
...
async function opAnalyzeImage(config, input, apiKey) {
  ...
  if (!imageUrl) return { success: false, error: "Sakana analyzeImage: 'imageUrl' is required.", skipped: true };
  ...
  return { result: data.choices?.[0]?.message?.content || "", model: data.model || model, tokensUsed: data.usage?.total_tokens || 0, provider: "sakana", operation: "analyzeImage" };
}
...
export default {
  async run(config, input, context = {}) {
    ...
    if (!handler) throw new Error(`Sakana: Unknown operation "${operation}". Valid: ${Object.keys(OPERATIONS).join(", ")}`);
