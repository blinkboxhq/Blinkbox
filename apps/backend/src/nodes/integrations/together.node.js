 ...
 * TOGETHER AI NODE
 *
 * OpenAI-compatible. Fast inference on open-source models.
 *
 * Output: { result, model, tokensUsed, provider: "together" }
 */
...
export default {
  async run(config, input, context = {}) {
    ...
    const cred = await resolveCredential(credentialId, context.workspaceId, "Together AI");
    ...
    try {
      ...
      if (err.response?.status === 401) throw new Error("Together AI: Invalid API key.");
      if (err.response?.status === 429) throw new Error("Together AI: Rate limit exceeded. Retry later.");
      throw new Error(`Together AI failed: ${err.response?.status || err.code} — ${err.message}`);
    }
  },
};
