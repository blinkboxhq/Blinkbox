        ...
        <span className="flex self-stretch -my-[5px] shrink-0">
          {Array.from({ length: depth + 1 }).map((_, i) => (
            <span key={i} className="relative w-3">
              <span className={`absolute inset-y-0 left-1/2 w-px ${i === depth ? "bg-white/[0.1]" : "bg-white/[0.05]"}`} />
            </span>
          ))}
        </span>
        {isBranch ? (
...
function ConfigurePanel({ node, updateConfig, renameNode }) {
  ...
  const actionList = !variant && (nodeDef?.category === "ai_models" || nodeDef?.category === "apps")
    ? NODE_ACTIONS[backendType] : null;
          ...
          {actionList?.length > 0 && (
            <div className="px-4 pt-3 pb-1">
              <ConfigSelect
                ...
                accentColor={nodeDef?.accentColor ? `rgb(${nodeDef.accentColor})` : "#6f97e8"}
              />
            </div>
          )}
          {eventGroup ? (
