...
// MiniMax M-series. M3 is natively multimodal + agentic with 1M context.
const MODELS_CHAT = ['MiniMax-M3', 'MiniMax-M2.7', 'MiniMax-M2.5', 'MiniMax-M2.1', 'MiniMax-M2'];
const MODELS_CODE = ['MiniMax-M3', 'MiniMax-M2.7'];
const MODELS_REASON = ['MiniMax-M3', 'MiniMax-M2.7'];
const MODELS_VISION = ['MiniMax-M3'];
...
function ChatPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="minimax" credentialId={config.credentialId} value={config.model} fallback="MiniMax-M3" onChange={(v) => updateConfig('model', v)} models={MODELS_CHAT} accentColor={MONO} />
      ...
      <Text label="Prompt" value={config.prompt} onChange={(v) => updateConfig('prompt', v)} placeholder="Ask MiniMax anything…" multiline nodeId={nodeId} />
...
function CodePanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="minimax" credentialId={config.credentialId} value={config.model} fallback="MiniMax-M3" onChange={(v) => updateConfig('model', v)} models={MODELS_CODE} accentColor={MONO} />
...
function ReasoningPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="minimax" credentialId={config.credentialId} value={config.model} fallback="MiniMax-M3" onChange={(v) => updateConfig('model', v)} models={MODELS_REASON} accentColor={MONO} />
...
function VisionPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      <ModelSelect provider="minimax" credentialId={config.credentialId} value={config.model} fallback="MiniMax-M3" onChange={(v) => updateConfig('model', v)} models={MODELS_VISION} accentColor={MONO} />
...
function DocumentPanel({ config, updateConfig, nodeId }) {
  return (
    <>
      ...
      <Text label="Document Text (optional — 1M-token context)" value={config.documentText} onChange={(v) => updateConfig('documentText', v)} placeholder="{{$node.content}} or leave blank" multiline nodeId={nodeId} />
    </>
  );
}
...
const ACTIONS = {
  'Chat Completion':         { op: 'message', desc: "Chat with MiniMax-M3 and the M-series",          Panel: ChatPanel },
  'Code Generation':         { op: 'code', desc: "Generate or explain code with MiniMax",             Panel: CodePanel },
  'Structured Output':       { op: 'structuredOutput', desc: "Return JSON matching a schema", Panel: StructuredPanel },
  'Function Calling':        { op: 'functionCalling', desc: "Let MiniMax call your tools/functions",  Panel: FunctionPanel },
  'Reasoning':               { op: 'reasoning', desc: "Deep step-by-step reasoning with MiniMax-M3",        Panel: ReasoningPanel },
  'Vision Analysis':         { op: 'analyzeImage', desc: "Describe or answer questions about an image",     Panel: VisionPanel },
  'Analyze Document':        { op: 'analyzeDocument', desc: "Answer questions about a 1M-token document",  Panel: DocumentPanel },
...
export default function MiniMaxNode({ config = {}, updateConfig, nodeId }) {
