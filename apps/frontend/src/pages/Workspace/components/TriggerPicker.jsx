...
export default function TriggerPicker() {
  ...
  const commit = useCallback((trigger) => {
    ...
    addNode({ id: newId, type: "custom", position, data: { backendType: trigger.backendType, label: trigger.label, type: "trigger", config } });
    playNodeLand();
    close();
    setSelectedNodeId(newId);
  }, [addNode, nodes, close, setSelectedNodeId, addNotification]);

  const selectTrigger = commit;
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === "Enter" && list[focusIdx]) selectTrigger(list[focusIdx]);
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [close, visibleTriggers, focusIdx, selectTrigger, page, search]);
