...
export default function TikTokTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="TikTok" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">TikTok</span>
      ...
      <div className="p-3 flex flex-col gap-3">
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                In TikTok Developer portal, configure webhook notifications to your BlinkBox URL.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <input value={config.appId || ''} onChange={e => updateConfig?.('appId', e.target.value)}
                placeholder="TikTok app ID" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <input value={config.clientSecret || ''} onChange={e => updateConfig?.('clientSecret', e.target.value)}
                placeholder="app client secret" type="password" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
          </>
        )}
        {tab === 'options' && (
          <div className="flex flex-col gap-1.5">
            ...
            <select value={config.event || 'all'} onChange={e => updateConfig?.('event', e.target.value)}
              ...
              <option value="comment.posted">Comment posted</option>
        ...
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.event','Event type'],['$trigger.videoId','Video ID'],['$trigger.videoUrl','Video URL'],['$trigger.description','Video description'],['$trigger.username','Creator username'],['$trigger.likesCount','Like count'],['$trigger.createdAt','When event occurred']].map(([k,d]) => (
