...
async function opCreateChannel(config, token) {
  ...
  const data = await bot(token, "post", p`/guilds/${config.guildId}/channels`, body);
...
async function opListChannels(config, token) {
  ...
  const data = await bot(token, "get", p`/guilds/${config.guildId}/channels`);
...
async function opGetChannel(config, token) {
  ...
  const c = await bot(token, "get", p`/channels/${config.channelId}`);
