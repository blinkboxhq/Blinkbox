...
async function opListAcl(config, token) {
  const res = await axios.get(`${BASE}/calendars/${cal(config)}/acl`, { headers: authHeaders(token), timeout: 120000 });
...
async function opUnshareCalendar(config, token) {
  ...
  await axios.delete(`${BASE}/calendars/${cal(config)}/acl/${encodeURIComponent(config.ruleId)}`, { headers: authHeaders(token), timeout: 120000 });
