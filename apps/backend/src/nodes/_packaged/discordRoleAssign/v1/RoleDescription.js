...
import { p } from "../GenericFunctions.js";
...
async function opAdd(config, client) {
  ...
  await client.put(p`/guilds/${config.guildId}/members/${config.userId}/roles/${config.roleId}`, {});
...
async function opRemove(config, client) {
  ...
  await client.del(p`/guilds/${config.guildId}/members/${config.userId}/roles/${config.roleId}`);
...
async function opListRoles(config, client) {
  ...
  const res = await client.get(p`/guilds/${config.guildId}/roles`);
...
async function opGetMember(config, client) {
  ...
  const res = await client.get(p`/guilds/${config.guildId}/members/${config.userId}`);
...
async function opCreateRole(config, client) {
  ...
  const res = await client.post(p`/guilds/${config.guildId}/roles`, body);
...
async function opDeleteRole(config, client) {
  ...
  await client.del(p`/guilds/${config.guildId}/roles/${config.roleId}`);
