...
export default function ZendeskTriggerNode({ config = {}, updateConfig }) {
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2A2A2A] bg-[#111111] rounded-t-xl">
        <div className="p-1 bg-[#222] rounded-md border border-[#333]">
          <img src={logo} className="w-3 h-3 object-contain" alt="Zendesk" />
        </div>
        <span className="text-[11px] font-semibold text-zinc-200 tracking-wide">Zendesk</span>
      ...
      <div className="p-3 flex flex-col gap-3">
          ...
          <>
            <div className="p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              <p className="text-[9px] text-zinc-400 leading-relaxed">
                In Zendesk → Admin → Business rules → Webhooks, add your BlinkBox URL.
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Subdomain</label>
              <input value={config.subdomain || ''} onChange={e => updateConfig?.('subdomain', e.target.value)}
                placeholder="mycompany" className="w-full bg-[#111] border border-[#222] rounded-md px-2.5 py-1.5 text-xs text-zinc-300 font-mono focus:outline-none transition-colors" />
            </div>
            <div className="flex flex-col gap-1.5">
        ...
        {tab === 'options' && (
          <div className="flex flex-col gap-1.5">
            <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Trigger Event</label>
            <select value={config.event || 'all'} onChange={e => updateConfig?.('event', e.target.value)}
              ...
              <option value="ticket_solved">Ticket solved</option>
            </select>
          </div>
        )}
        {tab === 'payload' && (
          <div className="flex flex-col gap-1.5">            <div className="flex flex-col gap-1 p-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg">
              {[['$trigger.event','Event type'],['$trigger.ticketId','Ticket ID'],['$trigger.subject','Ticket subject'],['$trigger.status','Ticket status'],['$trigger.priority','Priority level'],['$trigger.requesterEmail','Requester email'],['$trigger.assigneeName','Assigned agent name'],['$trigger.groupName','Support group name'],['$trigger.createdAt','Ticket creation time']].map(([k,d]) => (
