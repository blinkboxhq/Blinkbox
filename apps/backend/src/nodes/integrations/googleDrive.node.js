  ...
  async run(config, input, context = {}) {
    ...
    try {
      switch (operation) {
        ...
        case "getFile": {
          if (!config.fileId) throw new Error("Google Drive getFile: 'fileId' is required.");
        ...
        case "createFolder": {
          if (!config.name) throw new Error("Google Drive createFolder: 'name' is required.");
        ...
        case "uploadText": {
          if (!config.name || config.content === undefined) throw new Error("Google Drive uploadText: 'name' and 'content' are required.");
        ...
        case "downloadText": {
          if (!config.fileId) throw new Error("Google Drive downloadText: 'fileId' is required.");
        ...
        case "deleteFile": {
          if (!config.fileId) throw new Error("Google Drive deleteFile: 'fileId' is required.");
          await axios.delete(`${BASE}/files/${config.fileId}`, { headers: h(token), timeout: 15000 });
          return { deleted: true, fileId: config.fileId };
        }

        case "moveFile": {
          if (!config.fileId || !config.targetFolderId) throw new Error("Google Drive moveFile: 'fileId' and 'targetFolderId' are required.");
        ...
        case "shareFile": {
          if (!config.fileId || !config.email) throw new Error("Google Drive shareFile: 'fileId' and 'email' are required.");
