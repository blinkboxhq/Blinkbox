...
async function opListObjects(_config, client) {
  const { data } = await client.get(`/sobjects`, { timeout: 120000 });
...
async function opDescribeObject(config, client) {
  ...
  const { data } = await client.get(`/sobjects/${client.enc(config.objectType)}/describe`, { timeout: 120000 });
...
async function opGetRecentItems(config, client) {
  ...
  const { data } = await client.get(`/sobjects/${client.enc(config.objectType)}`, { timeout: 120000 });
...
async function opListApiVersions(_config, client) {
  ...
  const { data } = await axios.get(url, { headers: client.headers, timeout: 120000 });
